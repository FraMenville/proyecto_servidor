...
class Scraper:
    ...
    def _get_table(self) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            logger.error(f"Error al obtener la tabla en la sopa: {e}")
            return None
    ...
    def _get_rows(self) -> List[BeautifulSoup]:
        ...
        try:
            ...
            if not self.table:
                logger.debug("Tabla vacía o no hay tabla.")
                return []
            return self.table.find_all("tr", {"class": "ms-table_row"})
        except Exception as e:
            logger.error(f"Error al tratar de obtener rows de la tabla: {e}")
            return []
        
    def _parse_row(self, row: BeautifulSoup) -> Optional[Dict[str, str]]:
    ...
    def get_race_results(self, url: str) -> list:
        ...
        for row in rows:
            if not row.find('td'):
                logger.info("Fila sin datos, continuando...")
                continue
